...
package dataplexgetsearchsyntax
...
const kind string = "dataplex-get-search-syntax"
...
func (cfg Config) Initialize(srcs map[string]sources.Source) (tools.Tool, error) {
	...
	query := tools.NewStringParameterWithDefault("query", "type=projects/dataplex-types/locations/global/entryTypes/aspecttype", "Query to search for entries in Dataplex")
	name := tools.NewStringParameterWithDefault("name", fmt.Sprintf("projects/%s/locations/global", s.ProjectID()), "Name of the Dataplex Catalog to search in")
	page_size := tools.NewIntParameterWithDefault("pageSize", 100, "Maximum number of entries to return in a single page. Default is 100.")
	parameters := tools.Parameters{query, name, page_size}
...
func (t *SearchTool) Invoke(ctx context.Context, params tools.ParamValues) ([]any, error) {
	...
	page_size, _ := paramsMap["pageSize"].(int32)
